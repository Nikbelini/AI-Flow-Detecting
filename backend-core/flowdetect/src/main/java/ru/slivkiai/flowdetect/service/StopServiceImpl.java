...
public class StopServiceImpl implements StopService {
        ...
        private final PassengerFlowPredictionService predictionService;
        ...
        public List<StopResponse> getAllStopsByCityId(Long cityId) {
                ...
                List<PredictionResponseDto> predictions = predictionService.predict(cityId);

                Map<Long, Integer> predictionMap = predictions.stream()
                                .collect(Collectors.toMap(
                                                PredictionResponseDto::getStopId,
                                                PredictionResponseDto::getPredictedCount));
                return stops.stream().map(stop -> {

                        StopResponse resp = mapToResponse(stop);

                        if (stop.getUrl() == null) {
                                resp.setCount(predictionMap.get(stop.getId()));
                                resp.setPredicted(true);
                        } else {
                                resp.setPredicted(false);
                        }
